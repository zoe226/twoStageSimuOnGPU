...
void func_dataReshape_afterFFTR(float2* CoarseRangeFFT_ChirpNum_RangeSampleNum_MIMONum, float2* CoarseRangeFFT_ValidCoarseRangeBinNum_ChirpNum_RxNum_device, uint16_t* all_tx_seq_pos, uint16_t& TxNum, uint16_t& RxNum, uint16_t& TxReuseNum, uint16_t& CoarseRangeNum);
...
void func_abs_process(float* SpatialFFTA_ABS_AngleHorNum_yNum_VeloFFTNum_RangeNum_device, float2* Spatial_AngleHorNum_yNum_VeloFFTNum_RangeNum_device, uint16_t& AngleHorNum, uint16_t& VirtArrVertGridLen, uint16_t& VelocityNum, uint16_t CoarseRangeNum);
